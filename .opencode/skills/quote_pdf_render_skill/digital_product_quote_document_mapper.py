...
class DigitalProductQuoteDocumentMapper(QuoteDocumentMapper):
    def to_engineering_context(self) -> EngineeringPdfContext:
        ...
        if isinstance(summary, list) and summary:
            ...
            first.setdefault("product", first.get("sparePartsFee", ""))
            first.setdefault("serviceTaxRate", "")
            first.setdefault("productTaxRate", "")
            summary = [first]
        else:
            summary = [{"product": "", "other": "", "total": ""}]
        ...
        return base.model_copy(
            update={
                "form_data": form_data,
                "tax_rate": bool(
                    summary[0].get("productTaxRate") or summary[0].get("serviceTaxRate")
                ),
            }
        )
