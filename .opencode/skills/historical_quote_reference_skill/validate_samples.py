...
def validate_input_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "historical-quote-reference-input.schema.json"
    )
...
def validate_output_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "historical-quote-reference-output.schema.json"
    )
    validate(data, schema)
...
def main() -> None:
    ...
    print("historical reference sample contract check ok")
