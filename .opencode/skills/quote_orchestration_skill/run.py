...
from skill import dump_json, load_json, run_quote_workflow


BASE_DIR = Path(__file__).resolve().parent
INPUT_SCHEMA_PATH = BASE_DIR / "references" / "quote-orchestration-input.schema.json"
OUTPUT_SCHEMA_PATH = BASE_DIR / "references" / "quote-orchestration-output.schema.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the top-level quote orchestration workflow skill."
    )
    ...
    result = run_quote_workflow(payload)
    if not args.skip_schema_validation:
