...
def validate_input_sample() -> None:
    validate(
        _load_json(BASE_DIR / "samples" / "sample-input.json"),
        _load_schema(
            BASE_DIR / "references" / "quote-template-select-input.schema.json"
        ),
    )
...
def validate_output_sample() -> None:
    validate(
        _load_json(BASE_DIR / "samples" / "sample-output.json"),
        _load_schema(
            BASE_DIR / "references" / "quote-template-select-output.schema.json"
        ),
    )
...
def main() -> None:
    ...
    print("template selection sample contract check ok")
