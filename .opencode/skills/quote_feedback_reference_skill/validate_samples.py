...
def validate_input_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "quote-feedback-reference-input.schema.json"
    )
...
def validate_output_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "quote-feedback-reference-output.schema.json"
    )
    validate(data, schema)
...
def main() -> None:
    ...
    print("feedback reference sample contract check ok")
