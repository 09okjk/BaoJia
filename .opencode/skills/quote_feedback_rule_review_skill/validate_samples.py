...
def validate_input_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "quote-feedback-rule-review-input.schema.json"
    )
...
def validate_output_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "quote-feedback-rule-review-output.schema.json"
    )
    validate(data, schema)
...
def main() -> None:
    ...
    print("feedback rule review sample contract check ok")
