...
def validate_input_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "quote-review-output-input.schema.json"
    )
...
def validate_output_sample() -> None:
    ...
    schema = _load_schema(
        BASE_DIR / "references" / "quote-review-output-output.schema.json"
    )
    validate(data, schema)
...
def main() -> None:
    ...
    print("review output sample contract check ok")
