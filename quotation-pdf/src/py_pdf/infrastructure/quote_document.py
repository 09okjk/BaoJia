...
class QuoteDocumentJsonDataSource:
    _CURRENCY_SYMBOLS = {
        "USD": "$",
        "US$": "$",
        "RMB": "￥",
    ...
    _ZH_TRANSLATIONS = {
        ...
        "Transportation": "交通费",
        "Based on one round trip": "按一次往返计",
        "Accommodation": "住宿费",
        "Accommodation and meal arrangement during drydock attendance": "坞修期间住宿及餐费安排",
        "Hotel and meal arrangement if needed": "如需要，安排酒店及餐食",
        "Dockyard management fee": "船厂管理费",
        "Potential dockyard management / boarding / coordination fee during drydock": "坞修期间可能产生的船厂管理 / 登轮 / 协调费用",
        "Applicable if required by yard or local arrangement": "如船厂或当地安排要求，则适用",
        "Maritime reporting / safety permit": "海事申报 / 安全许可费用",
        "Potential hot work / permit / reporting related charge": "可能产生的动火许可 / 申报相关费用",
        "Applicable only if required by yard or local authority": "仅在船厂或当地主管机关要求时适用",
        ...
        "Pending confirmation for HCU overhaul scope": "HCU 大修范围待确认",
        "Pending confirmation for MPC inspection scope": "MPC 检查范围待确认",
        "Other repair if needed to be charged extra.": "如有其他维修项目，需另行收费。",
        "Other repair and extra scope if needed to be charged extra.": "如有其他维修或额外工程范围，需另行收费。",
        "Shipyard management fee / boarding pass / crane cost if needed to be charged as actual.": "如发生船厂管理费 / 登轮证 / 吊机费，将按实结算。",
        "Port entrance fee / other cost if needed to be charged as actual.": "如发生进港费或其他费用，将按实结算。",
        "Spares supply by shipside": "备件由船方提供。",
        "Commercial discount 5% applied within operator authority.": "在权限范围内给予 5% 商务折扣。",
        "Spare parts are excluded from this option and remain subject to separate confirmation.": "本方案不含备件，备件需另行确认。",
        "Quotation validity: 30 days": "报价有效期：30 天。",
        "Payment within 30 days upon invoice date.": "付款条件：发票日起 30 天内付款。",
        "All evaluation data is for quotation preparation only and actual execution is subject to site condition.": "本评估数据仅供报价准备参考，实际执行以现场情况为准。",
    ...
    def load(self) -> Mapping[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise QuoteDocumentError(
                f"Invalid JSON root: expected object ({self._json_path})"
            )
    ...
    def _extract_quote_document(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(candidate, dict):
            raise QuoteDocumentError(
                f"Invalid QuoteDocument structure: quote_document is not an object ({self._json_path})"
            )
    ...
    def _load_schema(self) -> dict[str, Any] | None:
        ...
        if not isinstance(schema, dict):
            raise QuoteDocumentError(
                f"Invalid schema root: expected object ({self._schema_path})"
            )
        return dict(schema)

    def validate_quote_document(self, quote_document: Mapping[str, Any]) -> None:
        ...
        try:
            ...
            location = path or "$"
            raise QuoteDocumentError(
                f"QuoteDocument schema validation failed at {location}: {exc.message}"
            ) from exc

    def to_quote_document_context(self) -> QuoteDocumentPdfContext:
        payload = self.load()
        quote_document = self._extract_quote_document(payload)
        self.validate_quote_document(quote_document)
        return QuoteDocumentPdfContext(quote_document=quote_document)
            ...
            for section in sections:
                ...
                for group in groups:
                    ...
                    if not numbered_service_section:
                        merged_rows = self._build_non_service_group_rows(
                            lines=lines,
                            group_title=group_title,
                            seen_pending_scopes=seen_pending_scopes,
                            section_type=section_type,
                        )
                        section_rows.extend(merged_rows)
                        continue

                    if group_label and numbered_service_section:
                        section_rows.append(
                            self._group_row(
                    ...
                    for line in lines:
                        ...
                        should_number_line = numbered_service_section and (
                            line_type == "priced"
                            or (line_type == "pending" and not has_priced_service_row)
                        )
                        row = self._line_to_description(
                            line,
                            group_no=group_no if numbered_service_section else "",
        ...
        if priced_line is not None:
            ...
            if pending_scope_key in seen_pending_scopes:
                ...
                merged_title = (
                    f"{merged_title}（待确认）"
                    if self._display_language != "en"
                    else f"{merged_title} (Pending confirmation)"
                )
    ...
    def _line_to_description(
        ...
        *,
        ...
        if isinstance(discount, dict):
            discount_text = str(discount.get("display", "") or "")
        ...
        if isinstance(remark_block, dict):
            ...
            if isinstance(items, list):
                for item in items:
                    ...
                    if normalized_text in payment_term_texts:
                        continue
                    if normalized_text in normalized_seen:
                        continue
                    normalized_seen.add(normalized_text)
    ...
    def _section_row(self, *, index: str, content: str) -> dict[str, Any]:
        return self._description_row(
            index=index,
            content=content,
            price="",
    ...
    def _should_show_section_title(self, section_title: str) -> bool:
        normalized = section_title.strip().lower()
        return normalized not in {"service", "spare parts", "other"}
    ...
    def _display_text(self, text: str) -> str:
        ...
        if option_prefix_match:
            ...
            translated_body = self._translate_composite_text(body)
            return f"{prefix}{translated_body}"
        if self._display_language == "auto":
            return self._translate_composite_text(stripped)
        if self._display_language == "zh":
            return self._translate_composite_text(stripped)
        return self._translate_composite_text(stripped)
    ...
    def _is_internal_scope_line(self, text: str) -> bool:
        ...
        for pattern in quantity_only_patterns:
        ...
        if re.search(r"\b(mt|fp|et|wp|rn)\d+\b", lowered):
            return True
